...
extension AttributedStrGenerator {
    class func generateEnterRoom(_ username : String) -> NSAttributedString {
        ...
        chatMsgStr.setAttributes([NSAttributedString.Key.foregroundColor : UIColor.orange], range: range)
    ...
    class func generateContent(_ username : String, _ content : String, _ font : UIFont) -> NSAttributedString {
        ...
        let userRange = NSRange(location: 0, length: username.count)
        chatMsgAttrStr.setAttributes([NSAttributedString.Key.foregroundColor : UIColor.orange], range: userRange)
        ...
        // 3.3.匹配结果
        let resultArray = regex.matches(in: chatMessage, options: [], range: NSRange(location: userRange.length, length: chatMessage.count - userRange.length))
    ...
    class func generateGift(_ username : String, _ giftName : String, _ giftURL : String) -> NSAttributedString {
        ...
        // 3.改变名称的颜色
        let userRange = NSRange(location: 0, length: username.count)
        giftChatAttrStr.setAttributes([NSAttributedString.Key.foregroundColor : UIColor.orange], range: userRange)
        
        // 4.改变礼物名称的颜色
        let giftRange = (giftMessage as NSString).range(of: giftName)
        giftChatAttrStr.setAttributes([NSAttributedString.Key.foregroundColor : UIColor(r: 102, g: 255, b: 255)], range: giftRange)
        
        // 5.礼物的图片
//        KingfisherManager.shared.cache.retrieveImageInDiskCache(forKey: giftURL, completionHandler: { result in
//
//        })
